 ...
 */
public class ProfesionalDAOImpl extends GenericDaoImpl<Profesional, Long>
		implements ProfesionalDAO {
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#buscarProfesional(java
	 * .util.HashMap, java.lang.Object)
	 */
	public List<Profesional> buscarProfesional(
			HashMap<String, Object> criterios, Object o) {
		Criteria criteria = getSession().createCriteria(Profesional.class);
		criteria.setProjection(Projections
				.groupProperty("experienciasLaborales"));
		return null;
	}
	 ...
	 */
	private Disjunction crearDisjuncionLike(String atributo,
			Collection<?> valores) {
		Disjunction disjunction = Restrictions.disjunction();
		for (Object valor : valores) {
			valor = UtilBean.transformarCriterioBusquedaSimple((String) valor);
			Criterion esp = Restrictions.ilike(atributo, (String) valor,
					MatchMode.ANYWHERE);
			disjunction.add(esp);
		}
		return disjunction;
	...
	private Disjunction crearDisjuncion(String atributo,
			Collection<?> valores) {
		Disjunction disjunction = Restrictions.disjunction();
		for (Object valor : valores) {
//			valor = UtilBean.transformarCriterioBusquedaSimple((String) valor);
	 ...
	 */
	public List<Profesional> buscarProfesional(
			HashMap<String, HashMap<String, List<?>>> criterios, List<?> ids) {
		...
		if (criterios.containsKey("especialidad")) {
			...
			criteria.createAlias("experienciasEspecialidad", "especialidad",
					Criteria.LEFT_JOIN);
		}
		if (criterios.containsKey("puesto") || criterios.containsKey("rol")
				|| criterios.containsKey("empresa")
				|| criterios.containsKey("sector")) {
			// experiencia laboral
			criteria.createAlias("experienciasLaborales", "experienciaLaboral",
					Criteria.LEFT_JOIN);
			...
			if (criterios.containsKey("empresa")) {
				criteria.createAlias("experienciaLaboral.empresa", "empresa",
						Criteria.LEFT_JOIN);
			}
		...
		if (criterios.containsKey("tipoFormacion")
				|| criterios.containsKey("nivelFormacion")
		...
		if (criterios.containsKey("idiomas")
				|| criterios.containsKey("nivelIdioma")) {
			// idiomas
			criteria.createAlias("idiomas", "idiomaProfesional",
					Criteria.LEFT_JOIN);
			criteria.createAlias("idiomaProfesional.idioma", "idioma",
					Criteria.LEFT_JOIN);
		}
...
//		}
		...
		while (it.hasNext()) {
			...
			HashMap<String, List<?>> campoValores = (HashMap<String, List<?>>) e
					.getValue();
			...
			if (e.getKey().toString().equals("especialidad")) {
				...
				// campoValores.get("descripcion"));
				Disjunction disjunction = Restrictions.disjunction();
				disjunction.add(Restrictions.in("especialidad.idOcupacion",
						campoValores.get("descripcion")));
				conjunctionQuery.add(disjunction);
			}
			if (e.getKey().toString().equals("rol")) {
				Disjunction djOcupacionEmpleo = Restrictions.disjunction();
				djOcupacionEmpleo.add(Restrictions.in(
						"experienciaLaboral.idOcupacion",
			...
			if (e.getKey().toString().equals("puesto")) {
				...
				djPuesto.add(Restrictions.in("experienciaLaboral.cargo",
			...
			if (e.getKey().toString().equals("empresa")) {
				conjunctionQuery.add(Restrictions.in("empresa.nombreComercial",
						campoValores.get("nombre")));
			}
			if (e.getKey().toString().equals("sector")) {
				Disjunction djSector = Restrictions.disjunction();
				djSector.add(Restrictions.in("experienciaLaboral.idSector",
						campoValores.get("descripcion")));
			...
			if (e.getKey().toString().equals("tipoFormacion")) {
				...
				djProfesion.add(Restrictions.in("formacion.idProfesion",
						campoValores.get("descripcion")));
			...
			if (e.getKey().toString().equals("nivelFormacion")) {
				...
				djNivel.add(Restrictions.in("formacion.idNivel",
			...
			if (e.getKey().toString().equals("camposEstudio")) {
				conjunctionQuery.add(Restrictions.in("formacion.campoEstudio",
						campoValores.get("camposEstudio")));
			}
			if (e.getKey().toString().equals("idiomas")) {
				conjunctionQuery.add(Restrictions.in("idioma.descripcion",
						campoValores.get("descripcion")));
			}
			if (e.getKey().toString().equals("nivelIdioma")) {
				conjunctionQuery.add(Restrictions.in(
						"idiomaProfesional.overall",
						campoValores.get("nivelIdioma")));
			}

			if (e.getKey().toString().equals("fechaNacimiento")) {
				List<Calendar[]> valor = (List<Calendar[]>) campoValores
						.get("fechaNacimiento");
				if (valor != null && !valor.isEmpty()) {
					...
					if (rangoFechaNacimiento != null) {
						...
						if (fechaInferior != null && fechaSuperior != null) {
							conjunctionQuery.add(Restrictions.between(
									"persona.fechaNacimiento",
									fechaInferior.getTime(),
									fechaSuperior.getTime()));
						} else {
							if (rangoFechaNacimiento[1] != null) {
								conjunctionQuery.add(Restrictions.ge(
										"persona.fechaNacimiento",
										rangoFechaNacimiento[1].getTime()));
							}
							if (rangoFechaNacimiento[0] != null) {
								conjunctionQuery.add(Restrictions.le(
										"persona.fechaNacimiento",
										rangoFechaNacimiento[0].getTime()));
							}
						}
					}
				}
			...
			if (e.getKey().toString().equals("profesional")) {
				List<Profesional> valor = (List<Profesional>) campoValores
						.get("profesional");
				if (valor != null && !valor.isEmpty()) {
					...
					if (profesionalBean != null) {
						...
						if (profesionalBean.getDisponibilidadTiempo() != null
								&& !profesionalBean.getDisponibilidadTiempo()
										.equals(0)) {
							conjunctionQuery.add(Restrictions.ge(
						...
						if (profesionalBean.getDisponibilidadContratacion() != null) {
							conjunctionQuery.add(Restrictions.eq(
									"disponibilidadContratacion",
									profesionalBean
											.getDisponibilidadContratacion()));
						}
						...
						if (profesionalBean.getPersona() != null) {
							...
							if (profesionalBean.getPersona().getNacionalidad() != null
									&& !profesionalBean.getPersona()
											.getNacionalidad().equals("0")) {
								conjunctionQuery.add(Restrictions
										.eq("persona.nacionalidad",
												profesionalBean.getPersona()
														.getNacionalidad()));
							}
							// Ciudad
...
//							}
							...
							if (profesionalBean.getPersona().getSexo() != null
									&& !profesionalBean.getPersona().getSexo()
											.equals(0)) {
								conjunctionQuery.add(Restrictions.eq(
										"persona.sexo", profesionalBean
												.getPersona().getSexo()));
							}
							// Estado civil
							if (profesionalBean.getPersona().getEstadoCivil() != null
									&& !profesionalBean.getPersona()
											.getEstadoCivil().equals(0)) {
								conjunctionQuery
										.add(Restrictions.eq(
												"persona.estadoCivil",
												profesionalBean.getPersona()
		...
		disjunctionQuery.add(Restrictions.in(
				"persona.numeroDocumentoIdentidad", ids));
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#buscarPreferenciasArea
	...
	public List<Area> buscarPreferenciasArea(Profesional profesional) {
		...
		List<Area> lista = getSession().createQuery(hql)
	...
	// String hql =
	// "select s from Preferencia p, Sector s where p.id.idPrfrnca = s.id and p.id.idPrfsnal = ? and p.id.idTpoPrfrnca = ?";
	...
	public List<Ubigeo> buscarPreferenciasCiudad(Profesional profesional) {
		...
		List<Ubigeo> lista = getSession().createQuery(hql)
				.setLong(0, profesional.getId())
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#buscarReferencias(pe
	...
	public List<Referencia> buscarReferencias(Profesional profesional) {
		...
		List<Referencia> lista = getSession().createQuery(hql)
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#obtenerIdiomas(pe.gob
	...
	public List<ProfesionalIdioma> obtenerIdiomas(Profesional profesional) {
		...
		List<ProfesionalIdioma> lista = getSession().createQuery(hql)
				.setLong(0, profesional.getId()).list();
		return lista;
	}
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#obtenerCertificaciones
	 * (pe.gob.trabajo.pcd.modelo.dominio.Profesional)
	 */
	public List<Certificacion> obtenerCertificaciones(Profesional profesional) {
		...
		List<Certificacion> lista = getSession().createQuery(hql)
				.setLong(0, profesional.getId()).list();
		return lista;
	}
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#obtenerReferencias(pe
	 * .gob.trabajo.silnet2.modelo.dominio.Profesional)
	 */
	...
	public List<ExperienciaEspecialidad> obtenerExperienciaEspecialidad(
		...
		List<ExperienciaEspecialidad> lista = getSession().createQuery(hql)
				.setLong(0, profesional.getId()).list();
		return lista;
	}
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#obtenerEmpleos(pe.gob
	 * .trabajo.silnet2.modelo.dominio.Profesional)
	 */
	public List<ExperienciaLaboral> obtenerEmpleos(Profesional profesional) {
		...
		ArrayList<ExperienciaLaboral> lista = (ArrayList<ExperienciaLaboral>) getSession().createQuery(hql).setLong(0, profesional.getId()).list();
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#borrarPreferencias(java
	 * .util.List, java.lang.Long, java.lang.Long)
	 */
	public int borrarPreferencias(List<Preferencia> preferencias, Long tipo,
			Long idProfesional) {
		int retorno = -1;
		...
		retorno = getSession().createQuery(hqlBorrar).setLong("tipo", tipo)
				.setParameterList("ids", listaId)
	 ...
	 * 
	 * @see
	 * pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#buscarPreferencias(java
	...
	public List<Preferencia> buscarPreferencias(Long tipo, Long idProfesional) {
		...
		return getSession().createQuery(hqlPreferencias).setLong("tipo", tipo)
				.setLong("idProfesional", idProfesional).list();
	}
	 ...
	 * .util.HashMap)
	 */
	public List<ReporteE1> getReporteE1(
			HashMap<String, HashMap<String, List<?>>> criterios,
		...
		while (it.hasNext()) {
			...
			if (e.getKey().toString().equals("noCalificado")) {
				noCalificado = (List<String>) campoValores
						.get("idNoCalificado");
			}
		}
		...
		List<Profesional> listaProfesionals = getSession().createQuery(hql)
				.setString("fechaInicio", fechaInicio)
		...
		for (Profesional prof : listaProfesionals) {
			...
			List<ExperienciaLaboral> lista = getSession().createQuery(hql2)
					.setLong(0, prof.getId()).setInteger(1, 1).list();
		...
		Long cantEmpresas = (Long) getSession().createQuery(query)
				.uniqueResult();
	...
	public List<ReporteE2> getReporteE2(
			HashMap<String, HashMap<String, List<?>>> criterios,
			String fechaInicio, String fechaFin) {
		...
		List<Profesional> listaProfesionals = getSession().createQuery(query)
				.list();
		...
		for (Profesional row : listaProfesionals) {
			...
			List<ExperienciaLaboral> lista = getSession().createQuery(query2)
					.setLong(0, row.getId()).setInteger(1, 1).list();

			for (ExperienciaLaboral experiencia : lista) {

				for (String d : grupo1) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG1(reporteE2
								.getCantidadOfertaG1() + 1);
					}
				}
				for (String d : grupo2) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG2(reporteE2
								.getCantidadOfertaG2() + 1);
					}
				}
				for (String d : grupo3) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG3(reporteE2
								.getCantidadOfertaG3() + 1);
					}
				}
				for (String d : grupo4) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG4(reporteE2
								.getCantidadOfertaG4() + 1);
					}
				}
				for (String d : grupo5) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG5(reporteE2
								.getCantidadOfertaG5() + 1);
					}
				}
				for (String d : grupo6) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG6(reporteE2
								.getCantidadOfertaG6() + 1);
					}
				}
				for (String d : grupo7) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG7(reporteE2
								.getCantidadOfertaG7() + 1);
					}
				}
				for (String d : grupo8) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG8(reporteE2
								.getCantidadOfertaG8() + 1);
					}
				}
				for (String d : grupo9) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG9(reporteE2
								.getCantidadOfertaG9() + 1);
					}
				}
				for (String d : grupo10) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG10(reporteE2
								.getCantidadOfertaG10() + 1);
					}
				}
				for (String d : grupo11) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG11(reporteE2
								.getCantidadOfertaG11() + 1);
					}
				}
				for (String d : grupo12) {
					if (experiencia.getIdOcupacion().equalsIgnoreCase(d)) {
						reporteE2.setCantidadOfertaG12(reporteE2
								.getCantidadOfertaG12() + 1);
					}
				}
			}
		}
	...
	public List<ReporteE3> getReporteE3(String fechaInicio, String fechaFin) {
		...
		resultado = (BigDecimal) getSession().createSQLQuery(query)
				.setParameter(0, sexo).setParameter(1, fechaInicio)
				.setParameter(2, fechaFin).setParameter(3, edad1)
				.setParameter(4, edad2).uniqueResult();
		...
		edad1 = 46;
		sexo = 1;
		resultado = (BigDecimal) getSession().createSQLQuery(query2)
				.setParameter(0, sexo).setParameter(1, fechaInicio)
				.setParameter(2, fechaFin).setParameter(3, edad1)
				.uniqueResult();
		if (resultado != null)
	...
	public List<ReporteE4> getReporteE4(
			HashMap<String, HashMap<String, List<?>>> criterios,
			String fechaInicio, String fechaFin) {
		...
		for (Profesional row : listaProfesionals) {
			...
			Long resultado = (Long) getSession().createQuery(query1)
					.setLong("idPersona", row.getId()).uniqueResult();

			if (resultado > 0) {

				String query2 = "select el from ExperienciaLaboral el where el.profesional.id = ? and el.principal = ?";

				List<ExperienciaLaboral> lista = getSession()
						.createQuery(query2).setLong(0, row.getId())
						.setInteger(1, 1).list();
		...
		resultado2 = (BigDecimal) getSession().createSQLQuery(query5)
				.setParameter(0, sexo).setParameter(1, fechaInicio)
				.setParameter(2, fechaFin).setParameter(3, edad1)
				.setParameter(4, edad2).uniqueResult();
		...
		edad1 = 46;
		sexo = 1;
		resultado2 = (BigDecimal) getSession().createSQLQuery(query6)
				.setParameter(0, sexo).setParameter(1, fechaInicio)
				.setParameter(2, fechaFin).setParameter(3, edad1)
				.uniqueResult();
		if (resultado2 != null)
	...
	public List<ReporteOferta> buscarPorRangoFecha(String fechaInicio, String fechaFin){
		...
		for(Profesional row : listaProfesionals){
			...
			fila.setFechaRegistro(new SimpleDateFormat("dd/MM/yyyy").format(
	...
	public List<Postulacion> obtenerPostulaciones(Profesional profesional) {
		...
		ArrayList<Postulacion> lista = (ArrayList<Postulacion>) getSession().createQuery(hql).setLong(0, profesional.getId()).list();
		return lista;
	}
	
	
	/* (non-Javadoc)
	 * @see pe.gob.trabajo.pcd.modelo.dao.ProfesionalDAO#buscarProfesional(java.util.List, java.util.List, java.util.Date, java.util.Date)
	 */
